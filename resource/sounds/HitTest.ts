...
class HitTest {
    ...
    /** 判断两物体是否发生碰撞（可调节精度） */
    public static
    complexHitTestObject(target1: egret.DisplayObject, target2: egret.DisplayObject, accuracy: number = 1): Boolean {
        return this.complexIntersectionRectangle(target1, target2, accuracy).width != 0;
    }

    /** 获取碰撞相交矩形区域 */
    public static
    intersectionRectangle(target1: egret.DisplayObject, target2: egret.DisplayObject): egret.Rectangle {
        // 如果有任一对象没加入显示列表，或者两对象hitTestObject的结果为false，则代表两对象没有发生碰撞
        if (!Tools.hitTest(target1, target2)) return new egret.Rectangle();

        // 分别得到两对象的显示矩形区域
        var bounds1: egret.Rectangle = target1.getBounds();
        var bounds2: egret.Rectangle = target2.getBounds();
