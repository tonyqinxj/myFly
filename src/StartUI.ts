class StartUI extends eui.Component implements eui.UIComponent {
    ...
    private star_snow_boat = []; // 减速飞机的怪物列表
    ...
    private init(): void {
        ...
        this.star_fly.forEach(star => {
            ...
            let line = star['line']
            line && line.parent && line.parent.removeChild(line)
    ...
    private onMainClick(e: egret.TouchEvent): void {
        ...
        if (GameData.UserInfo.guide == 1 && this.guide_index == 0) {
            this.guide_index = 1;
            this.updateGuide();
        }
    }
    ...
    private onSubClick(e: egret.TouchEvent): void {
        ...
        if (GameData.UserInfo.guide == 3 && this.guide_index == 0) {
            this.guide_index = 1;
            this.updateGuide();
        }
    }
    ...
    private onGoldClick(e: egret.TouchEvent): void {
        ...
        if (GameData.UserInfo.guide == 2 && this.guide_index == 0) {
            this.guide_index = 1;
            this.updateGuide();
        }
    }
    ...
    private onLevelUp1(e: egret.TouchEvent): void {
        switch (this.txt_up1.text) {
            ...
            default:
                ...
                if (GameData.UserInfo.guide == 2 && this.guide_index == 1) {
                    this.guide_index = 2;
                    this.updateGuide();
                }
    ...
    private onLevelUp2(e: egret.TouchEvent): void {
        switch (this.txt_up1.text) {
            case '射速':
                ...
                if (GameData.UserInfo.guide == 1 && this.guide_index == 1) {
                    this.guide_index = 2;
                    this.updateGuide();
                }
            ...
            case '强度':
                ...
                if (GameData.UserInfo.guide == 3 && this.guide_index == 2) {
                    this.guide_index = 3;
                    this.updateGuide();
                }
    ...
    private getGoldTime(ratio: number): void {
        ...
        GoldFx.playResult({
            x: this.txt_goldtime.x + this.gp_goldtime.x,
            y: this.txt_goldtime.y + this.gp_goldtime.y
        }, {x: this.img_gold_dest.x + this.gp_top.x, y: this.img_gold_dest.y + this.gp_top.y}, this);
    ...
    private starCreate(deltaTime): void {
        ...
        batch_info.init.forEach(conf => {
            if (conf.time <= this.game_time && conf.time > last_game_time) {
                ...
                let x = Tools.GetRandomNum(1, 749); //conf.x;
                let star_new = this.createStar(starConfig, conf['items'] || 0, conf.level, conf["blood"], {
                    x: x,
                    y: y
                }, dir, {
    ...
    private starMove(deltaTime): void {
        ...
        for (let i = 0; i < this.star_fly.length;) {
            ...
            // 无敌状态
            if (star.starConfig["unmatched"]) {
                this.checkUnmatched(star)
            }
    ...
    // 无敌状态
    private checkUnmatched(star: any): void {
        let config = star.starConfig;
        let info = config['unmatched']
        if (star['last_unmatched_time'] == undefined) star['last_unmatched_time'] = egret.getTimer();
        if (egret.getTimer() - star['last_unmatched_time'] > info.moveTime && !star['unmatched']) {
            star['last_unmatched_time'] = egret.getTimer();
            star['unmatched'] = true;
            star.model.playUn(true);

            let tr = new egret.Timer(info.unmatchedTime, 1);
            tr.addEventListener(egret.TimerEvent.TIMER_COMPLETE, () => {
                star['last_unmatched_time'] = egret.getTimer();
                star['unmatched'] = false;
                star.model.playUn(false);
            }, this);
            tr.start();
        }
    ...
    private checkSendStar(star: any): void {
        ...
        if (egret.getTimer() - star['last_send_star_time'] > moveTime && !star['sending']) {
            ...
            tr.addEventListener(egret.TimerEvent.TIMER, () => {
                this.sendOneStar(star)
            }, this);
            tr.addEventListener(egret.TimerEvent.TIMER_COMPLETE, () => {
                star['last_send_star_time'] = egret.getTimer();
                star['sending'] = false;
    ...
    private checkEat(): void {
        ...
        this.star_fly_eat.forEach(e => {
            ...
            if (!e['eatCount']) e.eatCount = 0;

            if (e.eatCount >= (eatInfo.maxEat || 10)) return;

            for (let i = 0; i < this.star_fly.length; i++) {

                if (e.eatCount >= (eatInfo.maxEat || 10)) break;
    ...
    private checkSnowBoat():void{
        this.snow_boats = 1;
        for(let i=0;i<this.star_snow_boat.length;i++){
            let star = this.star_snow_boat[i]
            let config = star.starConfig;
            let info = config['snow_boat']

            let dir:egret.Point = new egret.Point(
                star.model.x - this.boat_rect.x,
                star.model.y - this.boat_rect.y,
            );

            if(dir.length <= info.distance){
                this.snow_boats *= info.snowRatio
                // show 连线
                let line = star['line']
                if(line == undefined){
                    line = star['line'] = ResTools.createBitmapByName(info.line)
                    line.anchorOffsetX = line.width/2;
                    line.anchorOffsetY = line.height;
                }

                //line.height = dir.length - star.model.width/2*star.model.scaleX;
                line.height = dir.length;
                line.alpha = 0.85
                line.anchorOffsetY = line.height;
                line.rotation = myMath.angle(dir) + 90
                line.x = this.boat_rect.x;
                line.y = this.boat_rect.y;
                this.gp_layer_1.addChild(line);
            }else{
                // 隐藏连线，如果有
                let line = star['line']
                line && line.parent && line.parent.removeChild(line);
    ...
    private clear_star_fly_ex(star: any): void {
        ...
        for(let i=0;i<this.star_snow_boat.length;i++){
            if(this.star_snow_boat[i] == star){
                this.star_snow_boat.splice(i,1)
                break;
            }
        }
    }
    ...
    private createStar(starConfig: any, items: number, level: number, blood: number, pos: any, dir: any, info?: any): any {
        ...
        if (starConfig["eat"]) {
            ...
            star['eatCount'] = 0;
            model.scaleX = starConfig["eat"]["scale_init"]
            model.scaleY = starConfig["eat"]["scale_init"]
        }
        ...
        if(starConfig["snow_boat"]){
            this.star_snow_boat.push(star);
        }
    ...
    public onGoldOver(): void {
        if (this.last_add_boold_time == 0 || egret.getTimer() - this.last_add_boold_time > 200) {
            egret.Tween.get(this.img_game_gold_dest).to({scaleX: 1.22, scaleY: 1.22}, 100).to({
                scaleX: 1,
                scaleY: 1
            }, 70);
...
// 响应main的resume事件，目前只有复活一个环节需要
    public
    resume(type: string, shareok: boolean) {
